...
import org.example.Item.BattleItem;
import org.example.Item.ConsumableItem;
...
public class Game {
    ...
    public Game() {
        this.player = new Player("라이언", 160, 23);
    }
    ...
    private void battle() {
        ...
        System.out.println(player.name + "HP: " + player.hp + " 공격력: " + player.attack);

        Scanner sc = new Scanner(System.in);
        while (!monster.isDead() && !player.isDead()) {
            System.out.println("---------------------------------------\n");
            System.out.println("1. 공격 | 2. 방어 | 3. 인벤토리 | 4. 도망 | 5. 자동전투");
            System.out.print("선택: ");
            ...
            switch (choice) {
                ...
                case 5:
                    Turn turn = new Turn();
                    Autobattle autobattle = new Autobattle(player, monster, turn);
                    autobattle.start();
                    try {
                        autobattle.join();
                    } catch (InterruptedException e) {
                        System.out.println("전투가 중단되었습니다.");
                    }
            }
            if (!monster.isDead()) {
